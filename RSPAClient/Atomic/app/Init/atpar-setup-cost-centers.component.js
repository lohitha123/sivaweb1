"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var leftbar_animation_service_1 = require("../Home/leftbar-animation.service");
var SetupcostComponent = (function () {
    function SetupcostComponent(router, leftBarAnimationsewrvice) {
        this.router = router;
        this.leftBarAnimationsewrvice = leftBarAnimationsewrvice;
    }
    SetupcostComponent.prototype.homeurl = function () {
        this.leftBarAnimationsewrvice.isHomeClicked = false;
        this.leftBarAnimationsewrvice.isHide();
        this.router.navigate(['home']);
    };
    return SetupcostComponent;
}());
SetupcostComponent = __decorate([
    core_1.Component({
        moduleId: module.id,
        selector: 'setup-cost',
        templateUrl: './atpar-setup-cost-centers.component.html'
    }),
    __metadata("design:paramtypes", [router_1.Router,
        leftbar_animation_service_1.LeftBarAnimationService])
], SetupcostComponent);
exports.SetupcostComponent = SetupcostComponent;
//# sourceMappingURL=atpar-setup-cost-centers.component.js.map