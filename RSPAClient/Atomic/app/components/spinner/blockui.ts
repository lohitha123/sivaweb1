...
@Component({
    selector: 'atpar-blockUI',
    template: `
        <div class="ui-blockui ui-widget-overlay" [ngClass]="{'ui-blockui-document':!target}" [ngStyle]="{display: blocked ? 'block' : 'none'}"></div>
    `,
    styleUrls: ['./app/components/spinner/blockui.css',
        './app/components/spinner/theme.css',
        './app/components/spinner/sample.css',
        './app/components/spinner/temp.css'],
