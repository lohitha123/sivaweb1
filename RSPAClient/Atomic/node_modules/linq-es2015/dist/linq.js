...
class EnumerableImpl {
    ...
    Aggregate(seed, func = Constant.selfFn, resultSelector = Constant.selfFn) {
        let zero;
        let method;
        let selector;
    ...
    GroupJoin(inner, oKeySelect, iKeySelect, resultSelector = Constant.defGrouping) {
        return new EnumerableImpl(undefined, Generator.GroupJoin, [this,
            oKeySelect,
            resultSelector,
    ...
    ThenByDescending(keySelect = Constant.selfFn, equal = (a, b) => a - b) {
        if (this instanceof OrderedLinq) {
            let superEqual = this.equal;
            this.equal = (a, b) => {
                let result = superEqual(a, b);
...
function getKeyedMap(iterable, keySelector, selElement) {
    ...
    for (let value of iterable) {
        let key = keySelector(value);
        if (!key)
            throw exports.CONST_INVALID_KEY;
