...
if(box&&0<box.length){yield resultSelector(key,box);}}
...
continue;set.add(value);yield value;}}
exports.UnionFast=UnionFast;function*Join(target,oKeySelect,transform,map){for(let value of target){let key=oKeySelect(value);let innerSet=map.get(key);if('undefined'===typeof innerSet)
continue;for(let inner of innerSet){yield transform(value,inner);}}}
exports.Join=Join;function*GroupJoin(target,oKeySelect,transform,map){for(let value of target){let key=oKeySelect(value);let innerSet=map.get(key);if('undefined'===typeof innerSet)
continue;yield transform(value,innerSet);}}
...
exports.Zip=Zip;},{}],2:[function(require,module,exports){"use strict";class CSharpEnumerator{constructor(sourceIterator){this._iterator=sourceIterator;}
...
exports.CSharpEnumerator=CSharpEnumerator;},{}],3:[function(require,module,exports){"use strict";const Generator=require("./generators");const Constant=require("./utilities");const Iterator=require("./iterators");function getEnumerable(TSource){return new EnumerableImpl(TSource);}
...
Aggregate(seed,func=Constant.selfFn,resultSelector=Constant.selfFn){let zero;let method;let selector;if(Constant.CONST_FUNCTION===typeof seed){method=seed;selector=func;}
else{zero=seed;method=func;selector=resultSelector;}
let result=zero;for(let value of this){if(!result)
...
return true;}
Any(predicate){let iterator;if(!predicate&&(iterator=this._target[Symbol.iterator]())){return!iterator.next().done;}
...
OrderBy(keySelect=Constant.selfFn,equal=(a,b)=>a-b){return new OrderedLinq(this,(array)=>Generator.Forward(array),(a,b)=>equal(keySelect(a),keySelect(b)));}
OrderByDescending(keySelect=Constant.selfFn,equal=(a,b)=>a-b){return new OrderedLinq(this,(array)=>Generator.Reverse(array),(a,b)=>equal(keySelect(a),keySelect(b)));}
ThenBy(keySelect=Constant.selfFn,equal=(a,b)=>a-b){if(this instanceof OrderedLinq){let superEqual=this.equal;this.equal=(a,b)=>{let result=superEqual(a,b);return(0!=result)?result:equal(keySelect(a),keySelect(b));};return this;}
else{return new OrderedLinq(this,(array)=>Generator.Forward(array),(a,b)=>equal(keySelect(a),keySelect(b)));}}
ThenByDescending(keySelect=Constant.selfFn,equal=(a,b)=>a-b){if(this instanceof OrderedLinq){let superEqual=this.equal;this.equal=(a,b)=>{let result=superEqual(a,b);return(0!=result)?result:equal(keySelect(a),keySelect(b));};return this;}
else{return new OrderedLinq(this,(array)=>Generator.Reverse(array),(a,b)=>equal(keySelect(a),keySelect(b)));}}
...
class OrderedLinq extends EnumerableImpl{constructor(target,factory,equal){super(target,factory);this.equal=equal;}
[Symbol.iterator](){if(Constant.CONST_UNDEFINED===typeof this._factoryArg){this._factoryArg=this._target.ToArray();this._factoryArg.sort(this.equal);}
return this._factory(this._factoryArg);}}},{"./generators":1,"./iterators":2,"./utilities":4}],4:[function(require,module,exports){"use strict";exports.trueFn=()=>true;exports.selfFn=(o)=>o;exports.defGrouping=(a,b)=>{if(!b[exports.CONST_KEY]){b[exports.CONST_KEY]=a;}
return b;};function getDefaultVal(type,value=undefined){if(typeof type!==exports.CONST_STRING)
...
return undefined;}
exports.getDefaultVal=getDefaultVal;function getKeyedMap(iterable,keySelector,selElement){let map=new Map();for(let value of iterable){let key=keySelector(value);if(!key)
throw exports.CONST_INVALID_KEY;let group=map.get(key);if(!group){group=[];map.set(key,group);}
...
group.push(value);}
return map;}
exports.getKeyedMapFast=getKeyedMapFast;function getKeys(iterable,keySelector){let set=new Set();if(keySelector){for(let value of iterable){set.add(keySelector(value));}}
else{for(let value of iterable){set.add(value);}}
return set;}
