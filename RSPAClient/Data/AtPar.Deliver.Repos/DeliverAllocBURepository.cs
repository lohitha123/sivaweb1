...
{
    public class DeliverAllocBURepository : IDeliverAllocBURepository
    {
        ILog _log;
        public DeliverAllocBURepository(ILog log)
        {
            _log = log;
            _log.SetLoggerType(typeof(DeliverAllocBURepository));
        }

        #region GetBUnits
        public List<MT_DELV_BU_ALLOCATION> GetBUnits()
        {
            ...
            try
            {
                using (ATPAR_MT_Context objContext = new ATPAR_MT_Context())
                {
                    ...
                    sbSql.Append("SELECT BUSINESS_UNIT, FIRST_NAME+' '+MIDDLE_INITIAL+' '+LAST_NAME+'('+A.USER_ID+')' AS  ");
                    sbSql.Append("USERNAME, A.USER_ID FROM MT_DELV_BU_ALLOCATION A, MT_ATPAR_USER B WHERE A.USER_ID = B.USER_ID ");
                    ...
                    var lstBusinessUnits = objContext.Database.DifferedExecuteQuery<MT_DELV_BU_ALLOCATION>(fileds, sbSql.ToString()).ToList();
        ...
        private long DeleteAllBunits(string userID, ATPAR_MT_Context objContext)
        {
            ...
            try
            {
                ...
                sbSql.Append("DELETE FROM MT_DELV_BU_ALLOCATION WHERE USER_ID = '" + userID + "' ");
        ...
        private long InsertBunits(List<MT_DELV_BU_ALLOCATION> lstBUnits, string userID,
                                    string serverUserID, ATPAR_MT_Context objContext)
        {
            ...
            try
            {
                ...
                foreach (var item in lstBUnits)
                {
                    ...
                    sbSql.Append("INSERT INTO MT_DELV_BU_ALLOCATION(BUSINESS_UNIT, USER_ID, ");
        ...
        /// <param name="objContext"></param>
        /// <returns>Success or Failure code</returns>
        private long DeleteBUnits(List<MT_DELV_BU_ALLOCATION> lstBUnits, string userID, string serverUserID, ATPAR_MT_Context objContext)
        {
            var methodBaseName = this.GetType().FullName + "." + System.Reflection.MethodBase.GetCurrentMethod().Name + ": ";
            if (_log.IsDebugEnabled) { _log.Debug(methodBaseName); }
            ...
            try
            {
                ...
                foreach (var item in lstBUnits)
                {
                    ...
                    sbSql.Append("IF EXISTS(SELECT BUSINESS_UNIT FROM MT_DELV_BU_ALLOCATION WHERE  ");
                    sbSql.Append("USER_ID = '" + userID + "'AND BUSINESS_UNIT = '" + item.BUSINESS_UNIT + "') ");
                    sbSql.Append("DELETE FROM MT_DELV_BU_ALLOCATION WHERE USER_ID = '" + userID + "' ");
                    sbSql.Append("AND BUSINESS_UNIT = '" + item.BUSINESS_UNIT + "' AND UPDATE_USER_ID='" + serverUserID + "' ");
        ...
        public long ProcessBUnits(List<MT_DELV_BU_ALLOCATION> lstBUnits, string userID, string serverUserID)
        {
            ...
            try
            {
                using (ATPAR_MT_Context objContext = new ATPAR_MT_Context())
                {
                    using (DbContextTransaction trans = objContext.Database.BeginTransaction())
                    {
                        ...
                        long StatusCode = DeleteBUnits(lstBUnits, userID, serverUserID, objContext);
        ...
        public long ProcessSelectedBUnits(List<MT_DELV_BU_ALLOCATION> lstBUnits, string userID, string serverUserID)
        {
            string methodBaseName = string.Format("{0}.{1}:", this.GetType().FullName, System.Reflection.MethodBase.GetCurrentMethod().Name);
            if (_log.IsDebugEnabled) { _log.Debug(methodBaseName); }
