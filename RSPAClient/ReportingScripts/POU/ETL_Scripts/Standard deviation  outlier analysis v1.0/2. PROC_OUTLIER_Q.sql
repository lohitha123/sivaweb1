...
-- alter table CASEOUTLIERS add PROC_OUTLIER_Q varchar(1)

--- Create Stored procedure for Standard deviation outlier logic

create procedure PROC_OUTLIER_Q as 
begin
...
select [SPECIALTY_CODE],[PROCEDURE_CODE] FROM [CASEOUTLIERS] where SINGLE_PROC_OUTLIER_Q = 'N'
...
update CASEOUTLIERS SET PROC_OUTLIER_Q = CASE WHEN ([TOTAL_USAGE] < ABS((CASE WHEN ROUND(@newmeanX/@newstandardDeviationX,0) <= 2 THEN 2 ELSE 3 END)* @newstandardDeviationX) - @newmeanX) THEN 'Y' ELSE 'N' END FROM [CASEOUTLIERS] where [SPECIALTY_CODE] = @Specialty and [PROCEDURE_CODE] = @procedure
