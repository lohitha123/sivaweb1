...
-- alter table CASEOUTLIERS add PROC_OUTLIER_M varchar(1)


create procedure PROC_OUTLIER_M as 
begin
...
select [SPECIALTY_CODE],[PROCEDURE_CODE] FROM [CASEOUTLIERS] where SINGLE_PROC_OUTLIER_M = 'N'
...
update CASEOUTLIERS SET PROC_OUTLIER_M = CASE WHEN ([TOTAL_USAGE] < ABS((CASE WHEN ROUND(@newmeanX/@newstandardDeviationX,0) <= 2 THEN 2 ELSE 3 END)* @newstandardDeviationX) - @newmeanX) THEN 'Y' ELSE 'N' END FROM [CASEOUTLIERS] where [SPECIALTY_CODE] = @Specialty and [PROCEDURE_CODE] = @procedure
Fetch Next from SpecialtyCursor into @Specialty,@Procedure
end
