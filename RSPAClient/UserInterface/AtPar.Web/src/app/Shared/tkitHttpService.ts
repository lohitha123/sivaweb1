...
export class TkitHttpService {
    ...
    public checkSession() {
        try {
            TkitHttpService.redirect = "N";

            //If history cleared need to redirect to login
            let devicetoken = JSON.parse(localStorage.getItem("tkitDeviceTokenEntry"));
            if (devicetoken == null) {
            ...
            this.idleTime = parseInt(devicetoken[TokenEntry_Enum.IdleTime]);
            if (TkitHttpService.previousTime != null) {
                var time = new Date().getTime() - TkitHttpService.previousTime.getTime();
                let x = time / 60000;
                if (x >= this.idleTime) {
                    TkitHttpService.redirect = "Y";
                    // this.atParSharedDataService.storage = { "redirected": "Y"};
                    TkitHttpService.previousTime = new Date();
