...
const CUSTOM_INPUT_CONTROL_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => AtParTextComponent),
    multi: true
};
@Component({
    selector: 'atpar-text',
...
export class AtParTextComponent implements ControlValueAccessor, AfterViewInit {
    ...
    validatenumberswithZero: ATPAR_VALIDATION_RULES;
    ...
    validateNumberWithDot: ATPAR_VALIDATION_RULES;
    ...
    handelInput(e) {
        ...
        if (this.validationrules != null && this.validationrules != undefined && this.validationrules.length > 0) {
            ...
            this.validateNumberWithDot = this.validationrules.filter(x => x.ValidationType == "numeric_dot_initial")[0];
    ...
    onKey(event) {
        ...
        this.validatenumbers = this.validationrules.filter(x => x.ValidationType == "numeric_dot")[0];
        if (this.validatenumbers != null) {
            if (regExpValidator("numeric_dot", this.paramval)) {
                this.showerror = false;
                this.validatenumbers.status = true;
            }
            else {
        ...
        this.validatenumberswithZero = this.validationrules.filter(x => x.ValidationType == "numeric_dot_initial")[0];
        if (this.validatenumberswithZero != null) {
            if (regExpValidator("numeric_dot_initial", this.paramval)) {
                this.showerror = false;
