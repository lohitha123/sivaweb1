...
var DomHandler = (function () {
    ...
    DomHandler.prototype.relativePosition2 = function (element, target, option) {
        var elementDimensions = element.offsetParent ? { width: element.outerWidth, height: element.outerHeight } : this.getHiddenElementDimensions(element);
        var targetHeight = target.offsetHeight;
        var targetWidth = target.offsetWidth;
        var targetOffset = target.getBoundingClientRect();
        ...
        var top, left;
        console.log(option);
        if ((targetOffset.top + targetHeight + (elementDimensions.height + option)) > viewport.height)
            top = -1 * (elementDimensions.height + option);
        else
            top = targetHeight;
        if ((targetOffset.left + elementDimensions.width) > viewport.width)
            left = targetWidth - elementDimensions.width;
